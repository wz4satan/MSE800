...
class BasicAmenities:
    ...
    def serve_basic_refreshments(self):
        ...
        print(f"In-flight Service: Complimentary coffee, tea, and {self.snack_option} will now be served.")
        
    def distribute_reading_material(self):
        # Method 2: Distribute the Kia Ora magazine
        print("In-flight Service: The latest Kia Ora magazine is available in your seat pocket.")
        
    def show_safety_video(self):
        # Method 3: Show the signature Air NZ safety video
        print("Cabin Announcement: Please direct your attention to the screens for the Air New Zealand safety video.")
...
class DomesticFlight(GeneralFlight, BasicAmenities):
    ...
    def display_flight_info(self):
        ...
        print(f"Flight Type: Domestic | In-flight Wi-Fi: {'Available' if self.has_wifi else 'Not Available'}")
    ...
    def announce_baggage_claim(self):
        ...
        print(f"Cabin Announcement: Welcome to {self.arr_airport}. Please proceed to the domestic baggage claim area.")
...
class InternationalFlight(GeneralFlight, BasicAmenities):
    ...
    def display_flight_info(self):
        ...
        super().display_flight_info()
        print(f"Flight Type: International | Passport Required: {'Yes' if self.passport_req else 'No'}")
    ...
    def calculate_final_fare(self, flight_distance_km: float) -> float:
        ...
        final_cost = (base_cost + international_surcharge) * (1 + self.international_tax_rate)
        return round(final_cost, 2)
    
    def offer_duty_free_service(self):
        # Method 3: Offer duty-free shopping (specific to international)
        print("In-flight Service: Duty-free shopping is now available. Please check the catalogue in your seat pocket.")

    def verify_customs_clearance(self) -> bool:
        # Method 4 (Bonus): Verify customs clearance (specific to international)
        print(f"System Alert: Customs clearance data for {self.passenger_count} passengers has been sent to {self.arr_airport} Border Control.")
        return True